...
class Sync extends _Base
{
    ...
    public function room(){
        ...
        $this->response->SpaceSections =  $this->query('SELECT 
			"SpaceSectionID", "SpaceSectionName", COALESCE("Description", \'\') as "Description", "Order"
			FROM
        ...
        $this->response->SpaceSections = $this->group_record(@$this->response->SpaceSections, "SpaceSectionID", 
            'SELECT "SpaceSectionID", "SpaceID", "SpaceName", COALESCE("Description", \'\') as "Description", "Order" FROM
    ...
    public function user(){
        ...
        $this->response->Users = $this->query('SELECT 
            "UserID", "UserCode", "Fullname", "ShiftID", "Email", "UserTypeID", COALESCE("Description", \'\') as "Description", 
    ...
    public function promotion(){
        ...
        $this->response->GlobalDiscounts = $this->query('SELECT "DiscountID", "DiscountName",
            "StartDate", "EndDate", "StartHour", "EndHour", COALESCE("Description", \'\') as "Description", "PaymentMethodID", 
        ...
        $this->response->ServiceDiscounts = $this->group_record(@$this->response->ServiceDiscounts, [
            ...
            , 'SELECT
                ...
                null as "SubServiceCode",
                null as "SubServiceName",
                null as "SubPrice",
                null "SubServiceDuration",
                s."ServiceCode",
                s."ServiceName",
                s."ServiceID",
                s."Price",
                ...
                join "Service" s on s."ServiceID" = dd."ProductID"
                where 
                s."Archived" is null @key
            ',
            array(
    ...
    public function customer(){
        ...
        $this->response->Customers = $this->query('SELECT 
            "CustomerID", "CustomerName", "PhoneNumber", "Email", coalesce("Note", \'\') "Note", "DOB", 
            "Gender", "IDNumberType", "IDNumber", coalesce("CustomerCode", \'\') "CustomerCode", coalesce("LocalID", \'local-web-\'||"CustomerID") "LocalID", "Address"
            FROM
    ...
    public function product(){
        ...
        $this->response->Services = $this->query('SELECT 
            ...
            FROM
			"Service" where "BranchID" = :BranchID and "BrandID" = :BrandID
			and "Archived" is null 
        ', array(
            "BrandID"=>$this->request["BrandID"],
            "BranchID"=>$this->request["BranchID"]
        ...
        $this->response->SubServices = $this->group_record(@$this->response->SubServices, "SubServiceID", 
            'SELECT su."SubServiceID", su."UserID", u."UserCode", u."Fullname", coalesce("PriceModifier",0) "PriceModifier", coalesce("CommissionModifierPercent",0) "CommissionModifierPercent", coalesce("CommissionModifierValue",0) "CommissionModifierValue"
            FROM
			"ServiceUserMapping"  
