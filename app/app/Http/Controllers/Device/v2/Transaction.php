...
namespace Service\Http\Controllers\Device\v2;
...
class Transaction extends _Base
{
	...
	public function sales(){
        ...
        if(@$sales_id==null){
            ...
            [
                ...
                "CustomerPhone"=>@$sales_obj["CustomerPhone"],
                "VATPercentage"=>@$sales_obj["VATPercentage"],
            ...
            foreach ($sales_obj["Detail"] as $dtl) {
                ...
                switch (strtolower(trim($dtl["ProductType"]))) {
                    case 'service':
                        ...
                        $service_usage = $this->query('SELECT * FROM "ServiceInventoryUsage" where "ServiceID" = :id 
                        ', ["id"=>(float)@$dtl["ProductID"]]);

                        foreach ($service_usage as $u) {
                            $u->RefID = $dtlid;
                            $u->Meta = $service_detail;
                            $fullitemid[] = $u->ItemID;
                            $service_fullusage[] = $u;
                        }
                        ...
                        if(count($dtl["SubServices"]>0)){
                            ...
                            foreach ($dtl["SubServices"] as $subs) {
                                ...
                                ', ["id"=>(float)@$subservice_detail["SubServiceID"]]);
        ...
        if(count($fullusage)>0){
           ...
           $itrans = [
            ...
            foreach ($fullusage as $fi) {
                ...
                $idtl["NewStock"] = ((float)$im->CurrentStock) - abs((float)$idtl["Qty"]);
                $idtl["SubTotal"] = (float)$idtl["Price"] * abs((float)$idtl["Qty"]);
                $ivn = [
                    "CurrentStock" => $idtl["NewStock"]
                ];
                ...
                $this->upsert("InventoryTransactionDetail", $idtl);
                $itemsmeta[$fi->ItemID]->CurrentStock-=abs($idtl["Qty"]);
        ...
        if(count($service_fullusage)>0){
           ...
           $itrans = [
                ...
                "UserID"=> @$sales_obj["UserID"],
                "ReferenceID"=> $sales_id,
                "TransactionType"=> "Service Usages",
                "ReCalculatePrice"=> "0",
                "ReCalculateStock"=> "1",
                "RestrictDate"=> "0"
            ];
            ...
            foreach ($service_fullusage as $fi) {
                $im = $itemsmeta[$fi->ItemID];
                $idtl = [
                    "BrandID"=>@$this->request["BrandID"],
                    "BranchID"=>@$this->request["BranchID"],
    ...
    function get_transaction($object_clause,
        ...
        $object = $this->query('SELECT 
            ...
            "Void", "VoidBy", "VoidDate", "VoidDescription", "VoidLevel",
            "SalesTransactionNumber", "CustomerName","CustomerPhone", "CustomerID", "LocalID", "DiscountName", "DiscountID", "DiscountPercent", "VoidByType", "ClientVersion", "TransportVersion", "ReceivedTime", 
            "TotalDuration"
            FROM "SalesTransaction" '.$object_clause, $object_parameter);


         $object = $this->group_record(@$object, "SalesTransactionID"
            , ' SELECT "SalesTransactionID", "ServiceSalesTransactionDetailID" as "SalesDetailID", \'Service\' "ProductType", "ServiceID" as "ProductID", "ServiceCode" as "ProductCode",
                ...
                "CommissionPercent", "SpaceID", "StartedAt", "FinishedAt", "Void", "VoidBy", "VoidDate", "VoidDescription", "VoidLevel"
                ...
                "Discount", "DiscountPercent", "DiscountName", "SubTotal", "WorkerID", "WorkerName", "TotalCommission",
                "CommissionPercent", null "SpaceID", null "StartedAt", null "FinishedAt", "Void", "VoidBy", "VoidDate", "VoidDescription",  "VoidLevel"
        ...
        foreach ($object as $o) {
            foreach ($o->Detail as $d ) {
                ...
                if($d->ProductType == 'Service'){
                    ...
                    "LocalID", "FinishedAt", "Void", "VoidBy", "VoidDate", "VoidDescription",  "VoidLevel"
            ...
            $o->Customer = @$this->query('SELECT "CustomerID", "CustomerName", "PhoneNumber", "Email", 
                "Note", "DOB", "Gender", "IDNumberType", "IDNumber", 
                "Photo", "CustomerCode", "LocalID" from "Customer" where "CustomerID" = :id ',
                ["id"=>$o->CustomerID]
            )[0];
        }
    ...
    public function void()
    {
        ...
        if(count(@$this->request["Data"])>0){
            ...
            foreach ($this->request["Data"] as $d) {
                $d["VoidDescription"] = $this->coalesce(@$d["VoidDescription"], "");
                //later validate here
                $voiddata = [
                    "by"=>@$d["VoidBy"],
                    "id"=>@$d["ID"],
                ...
                switch ($d["Object"]) {
                    case 'SalesTransaction':
                        ...
                        $this->query('UPDATE "SalesTransaction" 
                                set "VoidBy" = :by,
                                "VoidLevel" = :level,
                                "VoidDate" = :date,
                                "VoidDescription" = :desc,
                                "Void" = \'1\'
                                where "BranchID" = :bid
                                and "SalesTransactionID" = :id
                                and ("Void" = \'0\' or "Void" is null or "Void" = \'N\')
                            ',
                            $voiddata
                        );

                        $this->query('UPDATE "ServiceSalesTransactionDetail" 
                                set "VoidBy" = :by,
                                "VoidLevel" = :level,
                                "VoidDate" = :date,
                                "VoidDescription" = :desc,
                        ...
                        $this->query('UPDATE "ItemSalesTransactionDetail" 
                                set "VoidBy" = :by,
                                "VoidLevel" = :level,
                                "VoidDate" = :date,
                                "VoidDescription" = :desc,
                        ...
                        $this->query('UPDATE "SubServiceSalesTransactionDetail" 
                                set "VoidBy" = :by,
                                "VoidLevel" = :level,
                                "VoidDate" = :date,
                                "VoidDescription" = :desc,
